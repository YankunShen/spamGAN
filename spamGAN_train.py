...
class RNNCritic(tf.keras.Model):
    def __init__(self, crit_config):
        ...
        self.dense = tf.keras.layers.TimeDistributed(tf.keras.layers.Dense(**crit_config['dense']))
...
def main(config = None):
    ...
    with g.as_default():
        ...
        with g.name_scope('pg_train'):
            ...
            #print("Before, pg_loss: {}".format(tf.rank(pg_loss)))
            #pg_test = tf.convert_to_tensor(())
            #print("Shape of (): {}".format(tf.rank(pg_test)))
            #pg_loss.set_shape(())
            #pg_loss.set_shape(tf.convert_to_tensor(()).shape)
            #print("After, pg_loss: {}".format(tf.rank(pg_loss)))
    ...
    def gen_run_epoch(sess, mode_string, writer, train_with_unsup = True):
        ...
        while True:
            try:
                ...
                break
        if mode_string == 'test':
            return {'perp' : total_perp/nexamples, 'loss' : total_loss/nexamples, "total_runtime": total_runtime}
    ...
    def clas_run_epoch(sess, mode_string, writer, clas_step):
        ...
        output = {'loss' : total_loss/nexamples, 'real_acc' : total_real_acc/nexamples,
                'fake_acc' : total_fake_acc/nexamples, 'step' : clas_step, 'total_runtime': total_runtime}
        if mode_string == 'test':
    ...
    with sess:
        ...
        with sum_writer:
            ...
            logger.info("\nTotal GPU runtime after generator pretrain: {}".format(total_runtime))
            ...
            logger.info("\nTotal GPU runtime after discriminator pretrain: {}".format(total_runtime))
            ...
            logger.info("\nTotal GPU runtime after classifier pretrain: {}".format(total_runtime))
            ...
            for e in range(config.adversarial_epochs):
                ...
                for i in range(config.gen_adv_epoch):
                    ...
                    total_runtime = total_runtime + gen_rtns["total_runtime"]
                logger.info("\nTotal GPU runtime after generator adv-train: {}".format(total_runtime)) 
                if config.mle_loss_in_adv:
                    for i in range(config.gen_mle_adv_epoch): 
                        ...
                        total_runtime = total_runtime + gen_rtns["total_runtime"]
                        logger.info('\nGen Val loss:{}'.format(gen_rtns['loss']))
                    logger.info("\nTotal GPU runtime after generator mle-train: {}".format(total_runtime))
                ...
                while config.discriminator_loss_lambda > 0 and disc_e < config.disc_adv:
                    ...
                    total_runtime = total_runtime + disc_rtns["total_runtime"]
                    disc_e += 1
                logger.info("\nTotal GPU runtime after discriminator adv-train: {}".format(total_runtime))
                # Generator validate
                ...
                logger.info("\nTotal GPU runtime after classifier adv-train: {}".format(total_runtime))
                checkpoint.save(sess, os.path.join(checkpoint_dir, 'ckpt-all'))
            ...
            test_runtime = clas_test_outputs["total_runtime"]
            logger.info("\nClassifier test result: \n{}".format(clas_test_outputs))
            
            logger.info("\nTest Generator: \n")
            gen_test_outputs = gen_run_epoch(sess, 'test', sum_writer, False)
            test_runtime = test_runtime + gen_test_outputs["total_runtime"]
            logger.info("\nGenerator test result: \n{}".format(gen_test_outputs))

            logger.info("\nTotal GPU runtime: {}".format(total_runtime))
            cpu_time = timer.GetCounter()
            cpu_time = cpu_time/1000.0
            print("Total CPU time: {} s".format(cpu_time) )

            dict_res = {
                "GPU_train_time" : total_runtime,
                "GPU_test_time"  : test_runtime,
                "CPU_time" : cpu_time,
                "f1"       : clas_test_outputs["real_f1"],
                "real_acc" : clas_test_outputs["real_acc"],
                "fake_acc" : clas_test_outputs["fake_acc"],
                "preplexity" : gen_test_outputs['perp'] 
            }
            return dict_res
